...
3. **Conflicting facts (X2; D152, D154).** A `conflicting` fact carries `candidates: [{value, evidence, state?}]` and a `conflict_kind`:
   - `candidates` (all values are extracted candidates) — **5b:** if the candidates give **different outcomes or different amounts**, the outcome is `needs_facts` ("which value is right?", both sources shown). **5c:** if every candidate gives the **same outcome and the same amount**, the outcome stands **capped at `likely_eligible`**; the fact is listed in `unconfirmed_decisive_facts`, the user is still asked to confirm, and only confirmation lifts the cap (D154 conditions 1 and 3).
   - `confirmed_vs_observed` or `confirmed_vs_confirmed` (a `user_confirmed` value contradicts an `observed` or another confirmed value) — **5a:** `manual_review`, even when every value gives the same answer, with an explanation naming both sources and what would resolve it (D154 condition 4).
   - **Deadlines never take a candidate's date** (D154 condition 2): a conflicting anchor gives `deadline.date = null` with status `disputed_anchor`; an extracted-only anchor gives status `unknown_anchor`. User-obligor paths add a labelled `advisory_act_by` from the **earliest** candidate; counterparty paths (carrier/seller/creditor duties) compute no due or overdue date until the anchor is resolved.
   - Every fixture file has a same-answer variant and a `confirmed_vs_observed` → `manual_review` case.
4. **Not yet due (D147(6), X3; D154).** A path that is not ripe yet is outcome **`not_yet_due`** with `reevaluate_at` (an ISO date) or `reevaluate_when` (a named event, e.g. "MBR filed"). It is never `not_eligible`. `not_eligible` means "not under this rule on these facts". When the trigger is something the user controls, the result also carries it as `next_action`, presented as something the user can do now, not a wait (D154). Contract rev 5.2 includes `not_yet_due`.
...
A fixture loader should map `likely_eligible_missing_evidence` → `likely_eligible`. `not_yet_due` (D147(6), contract rev 5.2) maps 1:1 and is never folded into `not_eligible`. The fixtures keep the mission's longer name so the expected meaning is explicit.
...
| Fact keys | `type`, `value` (always present; `null` when missing), `state`, optional `candidates` and `conflict_kind` (`candidates` \| `confirmed_vs_observed` \| `confirmed_vs_confirmed`), plus descriptive keys (`evidence`, `note`, `from`) |
...
| `expected` | exactly one of `outcome` or `results: [{path, outcome, …}]`; other keys are carried through for the test to assert (`deadline` with `date` (null when the anchor is unconfirmed or disputed), `status` (`unknown_anchor` \| `disputed_anchor`) and `advisory_act_by`; `missing_facts`, `unconfirmed_decisive_facts`, `assumptions`, `amount`, `deadline`, `claim`, `reevaluate_at`, `reevaluate_when`, `next_action`, `packet_readiness`, `dedupe`, `overlap`, `totals`, `forbidden_outputs`, `note`, …) |
