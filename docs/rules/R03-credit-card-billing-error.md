...
| Effective date | §1026.13 as amended 76 FR 79772 (2011-12-22) and 81 FR 84369 (2016-11-22); no later amendment in the eCFR source notes as of 2026-09-18 |
...
1. Payment was made on a **consumer** credit card or other **open-end consumer credit** plan. Not debit card, ACH, prepaid-asset, P2P balance, wire, or check → those are Regulation E or other regimes (`unsupported` for R03; route to R13). Business-purpose credit → `unsupported` (Reg Z applies to consumer credit).
...
|---|---|---|
| `unauthorized_charge` | (a)(1) | Consumer liability for unauthorized use is capped separately by 1026.12(b) ($50) — separate concept; not computed here |
...
| `credit_not_posted` (return/refund/payment not credited) | (a)(4) | |
...
- **Electronic notice / email:** counts as "written" **only if** the creditor's billing-rights statement stipulates that it accepts electronic billing-error notices **and** states the means (comment 13(b)-2). Therefore an **ordinary email to customer service does not preserve the formal notice right** unless that email channel is the stipulated means. A phone call is not written notice. Recoup must not claim otherwise.
...
| Boundary | "no later than 60 days after" → the notice is timely if **received** on or before anchor + 60 days (day 60 inclusive). No weekend/holiday roll-forward is provided (assumption A1: no extension). |
...
| Unknown anchor | no legal deadline computed; outcome `needs_facts`; display a **conservative act-by** date = `transaction_posting_date + 60` (the statement cannot have been transmitted before the charge posted), clearly labelled "earliest possible deadline — not the legal deadline". |
...
**P-1026.7(b)(9)** — 12 CFR 1026.7(b)(9)
...
- **L5 — BNPL.** Whether a given BNPL product is a "credit card" / open-end plan under Reg Z is unresolved. The CFPB's 2024 interpretive rule "Use of Digital User Accounts to Access Buy Now, Pay Later Loans, 89 FR 47068 (May 31, 2024)" is listed among the interpretive rules the CFPB withdrew on 2025-05-12 (FR 2025-08286, SHA-256 004541964d781c19e7a30fb76e71ebd8ac224cc59be527ceb84441bc45a0f8e0). → `unsupported` in v1.
...
7. Facts and evidence present → **`eligible`** (rule applies, window open). Evidence missing (e.g., no tracking for a non-delivery claim) → **`likely_eligible_missing_evidence`**.
