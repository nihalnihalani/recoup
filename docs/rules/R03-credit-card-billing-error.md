...
| Conflicting anchors (D152 / D154) | Never a firm deadline: `deadline.date = null`, status `disputed_anchor`, `advisory_act_by` = **earliest** candidate + 60 (labelled advisory). Outcome: (5b) candidates only and they give **different** outcomes or amounts → `needs_facts`; (5c) candidates only and every candidate gives the **same outcome and amount** → the outcome stands **capped at `likely_eligible`**, with the anchor in `unconfirmed_decisive_facts` until the user confirms it; (5a) a `user_confirmed` value contradicts an `observed` or another confirmed value → `manual_review`. An unconfirmed (candidate-only) anchor is `unknown_anchor`: no firm date, advisory act-by only. |
...
6. Anchor missing → **`needs_facts`** (+ advisory act-by, `unknown_anchor`). Anchor only extracted → no firm date (`unknown_anchor` + advisory act-by) and the outcome is capped at `likely_eligible`. Anchor conflicting → D152 5a/5b/5c as in §11 (`manual_review` / `needs_facts` / capped `likely_eligible`), always with `disputed_anchor` and an advisory act-by from the earliest candidate, never a firm date.
