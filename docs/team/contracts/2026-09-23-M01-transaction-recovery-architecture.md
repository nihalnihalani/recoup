...
- **Access (Revision 2, SEC-UP-5 / SEC-DEL-3): no `ctx.storage.getUrl` for evidence, anywhere** (queries, exports, logs, emails). `GET /evidence/file?id=<evidenceId>` (httpAction, M13) checks bearer auth, the tombstone and ownership. A missing, foreign or content-deleted id → 404 with an identical body. The action streams `ctx.storage.get(storageId)` with `Content-Disposition: attachment`, `X-Content-Type-Options: nosniff`, the **sniffed** content type and `Cache-Control: private, no-store`. The frontend fetches it with the JWT and renders previews through `URL.createObjectURL` (an `<img>` cannot send the header). PDFs are offered as downloads only and never rendered inline. Evidence queries return ids, docType, fileName, size, sha256 and extraction state, never a URL-shaped field (a test greps the query result for `/api/storage`).
