...
- 2026-09-21 · local + dev deployment · 496a916 · T20a (sonnet-tester): convex/testing.ts (seedUser via real `createAccount`, seedFixtures, lastCodeFor, resetUser; refuses without E2E_SEED_ENABLED and always on the production host) 18 tests; suite 51 files, 995 pass + 2 expected-fail (997) · LIVE on adorable-lion-138: E2E_SEED_ENABLED set, seedUser → verified user, fixtures created, resetUser cleared, re-seeded `e2e.lead@example.com` for the browser suite
- 2026-09-21 · local · d167816 · T13.1 (sonnet-integrations): F4 productName↔productName + threshold 0.2 (11-pair fixture) + confirm clears note; F5a foreign-currency stores unpriced; F7 auto path charges per-user; F8 reclaim stale claims after 15 min, refuse `bought`, content-length abort; F9 attempts reset; F11 WATCH_ROWS cap; F12 tombstone before spend; OUT_OF_STOCK_NOTE exported and cross-checked with src copy · suite 1023 (1018 pass, 2 expected-fail, 3 T12.1 in-flight)
- 2026-09-21 · local · 60d02c5 (+api.d.ts) · lead: `npm test` 51 files, 1024 pass + 2 expected-fail (1026); typecheck clean; codegen committed; `convex dev --once` deployed the clean tree to adorable-lion-138 · T05.2 (N1 identical string, N4 200/h global bucket, N5 format-before-existence, N8 restart, D102 capture) and T12.1 (F1 eligibleItems stamps ineligible items — now a mutation; F2 tombstoned rows bumped; F3 one claims query per purchase via `claims.by_purchase_type` + MAX_ITEMS_TOTAL 250, test at 60×50×12 asserts < 4096 ranges; F6 by_user_status; F5b per-currency totals; F10; F5a insights currency-scoped; D93 insights < 4,000 docs at 600 items; D87 fire/retryFailed; purchases.get coarse now; MARKET_MAX_ATTEMPTS 4)

## 2026-09-21 · wave 10 close · 6eeeb3e
- `npm test`: 65 files, 1286 passed, 1 expected fail (fairness.test.ts D74 single-tick fairness, known limitation F-D74-1).
- `npm run typecheck`, `npm run lint`: clean. `npx convex codegen`: no diff.
- `npx convex dev --once` → adorable-lion-138: "Convex functions ready".
