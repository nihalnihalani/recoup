...
| T09 | 1 | sonnet-tester | T05, T06 | yes | Tester acceptance suite for P01/P02 in separate files: full auth flow tests, alert gate matrix, unsubscribe route, crash-boundary and concurrent-duplicate simulations for the new sendDrop transaction, | convex/auth.test.ts, convex/notify.fault.test.ts, convex/alerts.flow.test.ts | in_progress |
