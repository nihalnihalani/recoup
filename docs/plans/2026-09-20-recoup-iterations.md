...
| W2 | **passed** 2026-09-20 | (this commit) | New watch with a target above today's price: first read hit the target, one alert claimed and sent through AgentMail to the account address, Drops list shows "Emailed". Dedupe (`watch:<id>:<cents>`), daily cap and failure rows are unit-tested; a second live check was blocked by the 10-minute cooldown, so the no-second-email path is proven in tests only. Charlie to confirm the email arrived in Gmail and not spam. |
| W3 | **passed** 2026-09-20 | (this commit) | "Find other stores" on the mixer: Best Buy $499.99, Amazon $479.00, KitchenAid $499.99, Walmart flagged "may be a different version". Confirming Amazon produced "Cheapest confirmed: amazon.com at $479.00". Found live: a page with no price was stored at $0.00; zero is now rejected everywhere prices are accepted. The search takes about two minutes (five sequential full-page reads). |
| W4 | **passed** 2026-09-20 | (this commit) | "I bought it" on a watch: became an active purchase, redirected to it, 14-day window counting down, price history carried over, confirmed policy reused without a re-fetch (M1 fix). Found live: the default date was tomorrow in UTC; now the shopper's local date. |
| T1 | done 2026-09-20 | `086205e` | New pitch line and trust copy on sign-in, README, `hackathon.md`, page title. Returns story removed from copy and from the example loader. |
| I7a production deploy | **passed** 2026-09-20 | (this commit) | Deployed `d88c5f8` plus `convex/budget.ts` to `cool-oyster-399`; site at https://cool-oyster-399.convex.site. Smoke test: four deep links 200, bundle points at the prod backend, unsigned webhook POST 401, auth discovery 200. Full loop on production with a fresh account: watch a real Target link ($499.99 read, target hit, alert sent from the shared alerts inbox), "I bought it" (14-day window), price check opened a $50.00 claim, draft written, first send auto-created the user's inbox and delivered to a controlled inbox, reply arrived by the production webhook and classified `promise` $50.00 with unresolved unchanged, credit confirmed, claim closed. Observed: right after "I bought it" a manual check is refused for a minute because the carried-over price read counts as a recent check. Alerts to Gmail land in spam on first contact (shared sender domain). Remaining: video, social post, submission (Charlie). |
